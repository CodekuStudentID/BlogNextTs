...
export default async function ASNPage({
  ...
  let sql = "SELECT * FROM posts WHERE (category LIKE '%ASN%' OR category LIKE '%CPNS%')";
  const params: any[] = [];
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            <div className="flex-1">
              <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter">
                Seputar <span className="text-blue-600">CPNS & ASN</span>
              </h1>
              <p className="mt-4 text-slate-500 max-w-xl">
                Menampilkan {posts.length} informasi terbaru mengenai seleksi ASN 2026.
              </p>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-10">
            ...
            {["Semua"].map((filter) => (
              <a 
                key={filter}
                href={`/asn?type=${filter}${query ? `&q=${query}` : ""}`}
          ...
          <div className="lg:col-span-8">
            {posts.length > 0 ? (
              ...
              <div className="text-center py-32 bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-200">
                ...
                <a href="/asn" className="text-blue-600 text-sm font-bold mt-2 inline-block underline">Lihat semua berita</a>
              </div>
            )}
          </div>
...
<div className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-blue-600 to-indigo-900 p-8 text-white shadow-xl shadow-blue-200">
  ...
  <div className="relative z-10">
    ...
    <h3 className="mb-3 text-2xl font-black leading-tight tracking-tight">
      E-Book Sukses <br />CPNS 2026
    </h3>
    
    <p className="mb-8 text-sm font-medium leading-relaxed text-blue-100/80">
      Dapatkan rangkuman materi TWK, TIU, dan TKP paling update berdasarkan kisi-kisi terbaru dari BKN.
    </p>
