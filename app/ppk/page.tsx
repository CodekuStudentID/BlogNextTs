...
export default async function PPKpage({
  ...
  let sql = "SELECT * FROM posts WHERE (category LIKE '%ASN%' OR category LIKE '%PPK%')";
  const params: any[] = [];
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            <div className="flex-1">
              <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter">
                Seputar <span className="text-blue-600">PPK</span>
              </h1>
              <p className="mt-4 text-slate-500 max-w-xl">
                Menampilkan {posts.length} informasi terbaru mengenai seleksi PPK 2026.
              </p>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-10">
            ...
            {["Semua"].map((filter) => (
              <a 
                key={filter}
                href={`/ppk?type=${filter}${query ? `&q=${query}` : ""}`}
          ...
          <div className="lg:col-span-8">
            {posts.length > 0 ? (
              ...
              <div className="text-center py-32 bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-200">
                ...
                <a href="/ppk" className="text-blue-600 text-sm font-bold mt-2 inline-block underline">Lihat semua berita</a>
              </div>
            )}
          </div>
          ...
          <aside className="lg:col-span-4 space-y-8">
            <div className="sticky top-24">
              <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white mb-8">
                ...
                <p className="text-slate-400 text-sm mb-6 leading-relaxed">Gabung komunitas pejuang PPK untuk tanya jawab dan simulasi soal terbaru.</p>
